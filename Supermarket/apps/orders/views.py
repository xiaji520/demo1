...
class OrderView(VerifyLoginView):
    ...
    def post(self, request):
        ...
        if payment.name == "支付宝":
            # 构造支付请求
            app_private_key_string = open(os.path.join(settings.BASE_DIR, "alipay/rsa_private_key.txt")).read()
            alipay_public_key_string = open(os.path.join(settings.BASE_DIR, 'alipay/rsa_alipay_pubulic_key.txt')).read()

            # 初始化对象
            alipay = AliPay(
                appid="2016092400582460",
            ...
            order_string = alipay.api_alipay_trade_wap_pay(
                ...
                subject="夏笈小店订单支付",  # 订单描述
                return_url="http://127.0.0.1:8081/orders/pay/",  # 同步通知地址
                notify_url="http://127.0.0.1:8081/orders/notify/"  # 异步通知地址 重要
            )
